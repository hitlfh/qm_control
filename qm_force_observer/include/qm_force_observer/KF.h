...
private:
    ...
    scalar_t pre_norm_force_ext_hat_EE;  // 用来对估计的外力进行低通滤波
    double alpha_ee = 0.1; // 这是低通滤波器的平滑因子，值越接近 1 表示滤波器响应当前值的影响越大，值越接近 0 表示更依赖于历史数据。
    scalar_t pre_norm_force_ext_hat_base;  // 用来对估计的外力进行低通滤波
    double alpha_base = 0.05; // 这是低通滤波器的平滑因子，值越接近 1 表示滤波器响应当前值的影响越大，值越接近 0 表示更依赖于历史数据。
